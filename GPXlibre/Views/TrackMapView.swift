...
///
/// Fix "remove-start-choice" (it14, Bloc 10) : la sélection tactile d'un départ personnalisé
/// ("Choisir le début", spec "per-track-settings") a été retirée — redondante depuis le
/// toggle A→B/B→A (it12), seule source de "où commence la trace" désormais. Ce composant
/// redevient un simple aperçu non interactif ; `TrackRideSettings.customStartPointIndex` et
/// `GPXTrack.reordered(using:)` restent INCHANGÉS (persistance existante non cassée — une
/// trace ayant déjà une valeur stockée continue de l'appliquer, seul le moyen de la DÉFINIR
/// depuis l'UI a disparu).
